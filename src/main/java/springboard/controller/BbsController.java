...
public class BbsController {
	...
	/*
	멤버변수이므로 클래스에서 전역적으로 사용할수있다. 해당 클래스의
	모든 command객체는 해당 인터페이스를 구현하여 정의한다.
	 */
	BbsCommandImpl command = null;
	/*
	@Autowired
		: 스프링설정파일에서 미이 생성된 빈을 자동으로 주입받고
		싶을때 사용, 타입을 기반으로 자동 주입되며, 만약 해당 타입의
		빈이 존재하지 않으면 에러발생되어 서버를 시작할수 없음.
		※생성자, 멤버변수, 메소드(setter)에 적용가능함
		-타입을 이용해 자동으로 프로퍼티값을 설정함.
		-해당 어노테이션은 멤버변수에만 적용할 수 있다. 함수내의
		지역변수에서는 사용할 수 없다.
		-타입을 통해 자동으로 설정되므로 같은 타입이 2개이상 존재하면
		예외가 발생됨.
	 */
	@Autowired
	...
	public String list(Model model, HttpServletRequest req) {
		...
		전달하기 위해 model에 저장한 후 매개변수로 전달한다.
		 */
