...
- int queryForInt(String sql)
	: 인파라티터가 있고, 쿼리문의 실행결과가 숫자를 반환하는 select계열의 쿼리문에 사용된다.
...
public class JDBCTemplateDAO {
	...
	//생성자
	/*
	컨트롤러에서 @Autowired를 통해 자동주입받았던 빈을 정적변수인
	...
	public ArrayList<SpringBbsDTO> list(Map<String, Object> map){
		 ...
		 String sql = ""
		      ...
		      sql += " ORDER BY bgroup DESC, bstep ASC ";//답변형일때 정렬방법
		      

		/*
		RowMapper 객체가 select를 통해 얻어온 ResultSet을 DTO객체에 저장하고
		List컬렉션에 적재하여 반환하게된다.
		 */
	...
	public void write(final SpringBbsDTO springBbsDTO) {
		...
		반드시 final로 선언하여 값의 변경이 불가능하게 해줘야한다.
		...
		template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				/*
				답변형 게시판에서 원본글인 경우 idx와 bgroup은 반드시 일치해야한다.
				리스트에서 게시물 정렬시 bgroup을 통해 order by절을 구성하기 때문이다.
				또한 nextval은 한문장에서 여러번 사용해도 같은 시퀀스를 반환한다. 
				 */
				String sql = "INSERT INTO springboard ("
						+ " idx, name, title, contents, hits, bgroup"
						+ " , bstep, bindent, pass) VALUES ("
	...
	public void updateHit(final String idx) {
		String sql = "UPDATE springboard SET hits=hits+1 "
				+ "WHERE idx=? ";
		//
	...
	public SpringBbsDTO view(String idx) {
		...
		/*
		queryForObject()는 반환결과가 0개이거나 2개이상인 경우
		예외가 발생되므로 반드시 예외처리를 해주는것이 좋다.
		*/
		try {
			//반환값이 하나만있는 select계열의 쿼리문
	...
	public void edit(final SpringBbsDTO dto) {
		String sql = "UPDATE springboard SET name=?, title=?, contents=?"
				+ " WHERE idx=? AND pass=?";
	...
	public void reply(final SpringBbsDTO dto) {
		...
		//write와 다른점은 bgroup에 기존게시물의 번호가 들어가는 것이다.
		String sql = "INSERT INTO springboard "
				+ "(idx, name, title, contents, pass, bgroup,bstep,bindent)"
	...
	/*
	답변들을 입력하기전 현제 step보다 큰 게시물들을 일괄적으로
